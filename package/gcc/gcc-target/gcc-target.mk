...
# Strip cc1 and cc1plus binaries to compact size (raw size is >200MB)
GCC_TARGET_LIBEXEC_DIR = $(TARGET_DIR)/usr/libexec/gcc/x86_64-buildroot-linux-gnu/$(GCC_TARGET_VERSION)
define GCC_TARGET_STRIP_CC1
        for cc1file in $(GCC_TARGET_LIBEXEC_DIR)/cc1*; do \
                if [ -f "$$cc1file" ]; then \
                        strip "$$cc1file" ; \
                fi ; \
        done
...
GCC_TARGET_POST_INSTALL_TARGET_HOOKS += GCC_TARGET_STRIP_CC1
...
define GCC_TARGET_RM_FILES
        ...
        rm -f $(TARGET_DIR)/usr/$(ARCH)-buildroot-linux-gnu/bin/*
        rm -f $(TARGET_DIR)/usr/bin/cpp
        rm -f $(TARGET_DIR)/usr/bin/c++filt
